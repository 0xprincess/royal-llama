...
def load(
    ...
    model_args: ModelArgs = ModelArgs(
        max_seq_len=max_seq_len, max_batch_size=max_batch_size, use_xformers=xformers, **params
    )
    ...
    ctx_tok_int8_quantize = default_quantize.set(int8_quantize)
    model = Transformer(model_args)
    default_quantize.reset(ctx_tok_int8_quantize)
    key_to_dim = {
        "w1": 0,
        "w2": -1,
        "w3": 0,
...
def main(
    ...
    generator = load(ckpt_dir, tokenizer_path, max_seq_len, max_batch_size, int8, xformers)
